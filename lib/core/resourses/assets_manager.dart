class AssetsManager {
  static const String IMAGE_PATH = "assets/images";
  static const String VECTOR_PATH = "assets/vectors";
  static const String ICONS_PATH = "assets/icons";
  static const String formateSvg = ".svg";
  static const String formatePng = ".png";



  // icons
  static const String googleIcon = "${ICONS_PATH}/google$formatePng";
  static const String macIcon = "${ICONS_PATH}/mac$formatePng";
  //Svg images
  static const String logo = "${VECTOR_PATH}/logo$formateSvg";
  static const String moon = "${VECTOR_PATH}/moon$formateSvg";
  static const String sun = "${VECTOR_PATH}/sun$formateSvg";
  static const String topPattern = "${VECTOR_PATH}/top_pattern$formateSvg";
  static const String bottomPattern = "${VECTOR_PATH}/bottom_pattern$formateSvg";
  static const String homeTopCard = "${VECTOR_PATH}/home_top_card$formateSvg";


  //png images
  static const String authBgImage = "${IMAGE_PATH}/auth_bg.png";
  static const String chooseModeBgImage = "${IMAGE_PATH}/choose_mode_bg.png";
  static const String homeArtistImage = "${IMAGE_PATH}/home_artist.png";
  static const String intoBgImage = "${IMAGE_PATH}/intro_bg.png";
}
